...
FastUsercallHookPtr<void(*)(task* tp), noret, rEDI> SetupCamPathCam2_Ruin_h(0x5E5D10);
...
static auto Camera_PathLinearScanXZ = GenerateUsercallWrapper<Sint32(*)(taskwk*, NJS_POINT3*, NJS_POINT3*, NJS_POINT3*, Float*)>(rEAX, 0x5E5040, rEAX, rECX, rEDI, rESI, stack4);

static VariableHook<Sint32, 0x3C7ED88> Paths_m;
...
static BOOL CameraExec_m(taskwk* twp, taskwk* ptwp, taskwk* ctwp)
{
	...
	if (pathwk->modeflag & PATH_MODE_XZ)
	{
		camwk->frame = ScanNearPath2LXZ(camwk->frame, 280, &postgt, camwk->pospath, camwk->pathnum, NULL, &posnear, &vecnear);
	}
	else
	{
		camwk->frame = ScanNearPath2L(camwk->frame, 280, &postgt, camwk->pospath, camwk->pathnum, NULL, &posnear, &vecnear);
	}
	...
	if (pathwk->modeflag & PATH_MODE_XZ)
	{
		camwk->frame_back = ScanNearPath2LXZ(camwk->frame_back, 280, &tgtback, camwk->pospath, camwk->pathnum, NULL, &posnear_back, NULL);
	}
	else
	{
		camwk->frame_back = ScanNearPath2L(camwk->frame_back, 280, &tgtback, camwk->pospath, camwk->pathnum, NULL, &posnear_back, NULL);
	}
	...
	if (pathwk->modeflag & PATH_MODE_XZ)
	{
		camwk->frame_forward = ScanNearPath2LXZ(camwk->frame_forward, 280, &tgtforward, camwk->pospath, camwk->pathnum, NULL, &posnear_forward, NULL);
	}
	else
	{
		camwk->frame_forward = ScanNearPath2L(camwk->frame_forward, 280, &tgtforward, camwk->pospath, camwk->pathnum, NULL, &posnear_forward, NULL);
	}
		...
		if (!IsCompulsionCamera_m(pnum) && !IsEventCamera_m(pnum) && twp->wtimer > 20)
		{
			...
			if ((camwk->pospath || Paths_m[pnum] <= 2) && CheckRangeIn(&rangein, &twp->pos, &ptwp->pos, &twp->ang))
			{
				if (!camwk->pospath)
				{
					...
					camwk->pospath = (NJS_POINT3*)MAlloc(sizeof(NJS_POINT3) * 14 * (nbPoint + 3));
					...
					camwk->pathnum = CalcSpline(&spline_data) - 45.0; // weird
					camwk->pospath += 15;
