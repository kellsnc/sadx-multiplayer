...
static UsercallFuncVoid(SetupCamPathCam2_h, (task* tp), (tp), 0x5C32D0, rEDI);
...
static auto Camera_PathLinearScan = GenerateUsercallWrapper<Sint32(*)(taskwk*, NJS_POINT3*, NJS_POINT3*, NJS_POINT3*, Float*)>(rEAX, 0x5C2590, rEAX, rECX, rEDI, rESI, stack4);
static auto Camera_PathLinearScanXZ = GenerateUsercallWrapper<Sint32(*)(taskwk*, NJS_POINT3*, NJS_POINT3*, NJS_POINT3*, Float*)>(rEAX, 0x5C25F0, rEAX, rECX, rEDI, rESI, stack4);

static VariableHook<Sint32, 0x3C74EA4> Paths_m;
...
static BOOL CameraExec_m(taskwk* twp, taskwk* ptwp, taskwk* ctwp)
{
	...
	if (pathwk->modeflag & PATH_MODE_XZ)
	{
		camwk->frame = ScanNearPath2LXZ(camwk->frame, 300, &postgt, camwk->pospath, camwk->pathnum, NULL, &posnear, &vecnear);
	}
	else
	{
		camwk->frame = ScanNearPath2L(camwk->frame, 300, &postgt, camwk->pospath, camwk->pathnum, NULL, &posnear, &vecnear);
	}
	...
	if (pathwk->modeflag & PATH_MODE_XZ)
	{
		camwk->frame_back = ScanNearPath2LXZ(camwk->frame_back, 300, &tgtback, camwk->pospath, camwk->pathnum, NULL, &posnear_back, NULL);
	}
	else
	{
		camwk->frame_back = ScanNearPath2L(camwk->frame_back, 300, &tgtback, camwk->pospath, camwk->pathnum, NULL, &posnear_back, NULL);
	}
	...
	if (pathwk->modeflag & PATH_MODE_XZ)
	{
		camwk->frame_forward = ScanNearPath2LXZ(camwk->frame_forward, 300, &tgtforward, camwk->pospath, camwk->pathnum, NULL, &posnear_forward, NULL);
	}
	else
	{
		camwk->frame_forward = ScanNearPath2L(camwk->frame_forward, 300, &tgtforward, camwk->pospath, camwk->pathnum, NULL, &posnear_forward, NULL);
	}
...
static void sub_4F9A60_m(int pnum)
{
...
static void ObjectCamPathCam_Exec_m(task* tp)
{
	...
	if (twp->btimer & 1)
	{
		...
		else if (!CameraExec_m(twp, ptwp, nullptr))
		{
			sub_4F9A60_m(pnum);
			twp->wtimer = 0;
			twp->btimer &= ~1;
	...
	else
	{
		if (!IsCompulsionCamera_m(pnum) && !IsEventCamera_m(pnum) && twp->wtimer > 20)
		{
			...
			if ((camwk->pospath || Paths_m[pnum] <= 5) && CheckRangeIn(&rangein, &twp->pos, &ptwp->pos, &twp->ang))
			{
				if (!camwk->pospath)
				{
					int nbPoint = PATH(twp)->points - pathwk->nBottomPathGap - pathwk->nTopPathGap;
					camwk->pospath = (NJS_POINT3*)MAlloc(sizeof(NJS_POINT3) * 15 * (nbPoint + 3));
					...
					spline_data.rate = 0.06666667f; // 1/15
					spline_data.dst = camwk->pospath;
					spline_data.spline_type = 1;
					spline_data.variety = 0;
					camwk->pathnum = CalcSpline(&spline_data) - 48.0; // weird
					camwk->pospath += 16;
...
static void SetupCamPathCam2_r(task* tp)
{
	if (multiplayer::IsEnabled())
	{
		SetupCamPathCam2_h.Original(tp);
		...
		for (int i = 1; i < PLAYER_MAX; ++i)
		{
			...
			SetupCamPathCam2_h.Original(tp2);
...
{
	SetupCamPathCam2_h.Hook(SetupCamPathCam2_r);
}
