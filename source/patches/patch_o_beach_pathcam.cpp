...
static auto Camera_PathLinearScanXZ = GenerateUsercallWrapper<Sint32(*)(taskwk*, NJS_POINT3*, NJS_POINT3*, NJS_POINT3*, Float*)>(rEAX, 0x4F9010, rEAX, rECX, rEDI, rESI, stack4);

static VariableHook<Sint32, 0x3C5E440> Paths_m;
		...
		if (!IsCompulsionCamera_m(pnum) && !IsEventCamera_m(pnum) && twp->wtimer > 20)
		{
			...
			if ((camwk->pospath || Paths_m[pnum] <= 2) && CheckRangeIn(&rangein, &twp->pos, &ptwp->pos, &twp->ang))
			{
				if (!camwk->pospath)
				{
					...
					camwk->pathnum = CalcSpline(&spline_data) - 18;
					camwk->pospath += 6;
...
static void SetupCamPathCam2_Beach_r(task* tp)
{
	if (multiplayer::IsEnabled())
	{
		SetupCamPathCam2_Beach_h.Original(tp);
		...
		for (int i = 1; i < PLAYER_MAX; ++i)
		{
			...
			SetupCamPathCam2_Beach_h.Original(tp2);
